...
<div class="data_list">
		...
		<form class="select_form" name="myForm" method="post" action="dormBuild?action=addManager">
				...
				<span class="data_select">
					...
					<option value="">请选择售药员...</option>
					<c:forEach var="managerToSelect" items="${managerListToSelect }">
						<option value="${managerToSelect.drugSellerId }">${managerToSelect.name }</option>
					</c:forEach>
					</select>
			...
			<table class="table table-hover table-striped table-bordered">
				...
				<c:forEach  varStatus="i" var="managerWithId" items="${managerListWithId }">
					<tr>
						...
						<td>${managerWithId.gender }</td>
						<td>${managerWithId.tel }</td>
						<td><button class="btn btn-mini btn-danger" type="button" onclick="dormManagerMove(${managerWithId.drugSellerId},${managerWithId.counterId})">移除</button></td>
					</tr>
				</c:forEach>
			</table>
		</div>
